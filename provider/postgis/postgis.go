...
func (p *Provider) TileFeatures(ctx context.Context, layer string, tile provider.Tile, fn func(f *provider.Feature) error) error {
	...
	//should count first if too much features, cancle buffer first, then limit features
	...
	for rows.Next() {
		...
		if i >= limit {
			return fmt.Errorf("too much features")
		}
		i++
	}
	log.Printf("features: %d, takes:%v", i, time.Since(st))
	return rows.Err()
}
