...
func (df *Datafile) dataImport(dp *DatafileBind) (*Task, error) {
	task := &Task{
		ID:   df.ID,
		Type: "dsimport",
	}
	...
	if df.Overwrite {
		...
		err = createTable(df.ID, df.Fields, df.Geotype)
		if err != nil {
	...
	case ".csv":
		...
		ix := prepIndex(headers, dp.Lon)
		iy := prepIndex(headers, dp.Lat)
		isgeom := (ix != -1 && iy != -1)
		if isgeom {
	...
	case ".geojson", ".json":
		...
		go func(task *Task, features []json.RawMessage) {
			task.State = "processing"
			t := time.Now()
			var vals []string
...
func (df *Datafile) ogrImport() (*Task, error) {
	task := &Task{
		ID:   df.ID,
		Type: "dsimport",
	}
